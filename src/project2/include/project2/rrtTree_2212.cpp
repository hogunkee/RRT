...
int rrtTree::generateRRT(double x_max, double x_min, double y_max, double y_min, int K, double MaxStep) {
	...
	int check=0;
	while(true){
		if ((iter_count > K && check >=20) || iter_count++ > 40000 || count > 40000)// && check >= 1 && iter_count < 30000)
			break;
		...
		if (near_idx != -1 && newState(out, near_node->location, x_rand, MaxStep)==0){
			...
			if (tmp->idx_parent!=NULL && tmp->idx_parent!=0){
				...
				if(dist<4 && newState(out, tmp->location, x_new, 100)==0){
...
std::vector<traj> rrtTree::backtracking_traj(){
	...
	if (near_index == -1)
		near_index = nearestNeighbor(x_goal);
	//0--------------------------------------------------0//
