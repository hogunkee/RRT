...
void rrtTree::addVertex(point x_new, point x_rand, int idx_near, double alpha, double d) {
	...
	weight[count++] = weight[idx_near] + 1;
...
int rrtTree::generateRRT(double x_max, double x_min, double y_max, double y_min, int K, double MaxStep) {
	...
	while(true){
		...
		//printf("start\n");
		//printf("%d\n",nearestNeighbor(x_rand, MaxStep));
		node* x_near;
		int near_idx = nearestNeighbor(x_rand, MaxStep);
		if (near_idx != -1)
			x_near = ptrTable[nearestNeighbor(x_rand, MaxStep)];
		//printf("end\n");
		if (near_idx != -1 && newState(out, x_near->location, x_rand, MaxStep)==0){
			point x_new;
			x_new.x = out[0];
			x_new.y = out[1];
			x_new.th = out[2];
...
int rrtTree::newState(double *out, point x_near, point x_rand, double MaxStep) {
	...
	if (alpha > 0){
		...
		y_ = x_near.y - (cos(x_near.th + beta) - cos(x_near.th)) * L / tan(alpha);
		/*
		x_ = x_near.x - 2*L*sin(beta/2)*cos(x_near.th + beta/2)/tan(alpha);
		y_ = x_near.y + 2*L*sin(beta/2)*sin(x_near.th + beta/2)/tan(alpha);
		*/
	} else{
		x_ = x_near.x + 2*L*sin(beta/2)*cos(x_near.th + beta/2)/tan(alpha);
		y_ = x_near.y - 2*L*sin(beta/2)*sin(x_near.th + beta/2)/tan(alpha);
	...
	//y_ = x_near.y + 2*R*sin(beta/2)*sin(x_near.th + beta/2);
...
bool rrtTree::isCollision(point x1, point x2, double d, double R) {
	...
	for (int k=0; k<num_step; k++){
        ...
        i = x_ / res + map_origin_x;
        j = y_ / res + map_origin_y;
