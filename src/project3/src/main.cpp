...
int main(int argc, char** argv){
    ...
    while(running){
        ...
        case RUNNING: {
			...
			double speed = (1.0 - fabs(ctrl))*3 + 1.5;
...
void generate_path_RRT()
{
	printf("start generate rrt");
	waypoints.at(0).th = robot_pose.th;
	printf("theta setting");
	rrtTree tmp_rrt;
	for(int i = 0; i < waypoints.size()-1; i++){
		printf("for i");
		tmp_rrt = rrtTree(waypoints.at(i), waypoints.at(i+1), map, map_origin_x, map_origin_y, res, margin);
		printf("rrtTree initialize");
		tmp_rrt.generateRRT(world_x_max, world_x_min, world_y_max, world_y_min, K, MaxStep);
		printf("rrtTree generate");
