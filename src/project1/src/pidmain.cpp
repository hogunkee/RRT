...
int main(int argc, char** argv){
	...
	int count=1;	//count time
    // control rate, 10 Hz
    ros::Rate control_rate(10);
    while(ros::ok()){
		//check whether pioneer reached the check point
		point g = path[current_goal];
		float distance = pid_ctrl.get_distance(car_pose, path[current_goal]);
		if (distance < 0.2){
			printf("-------------------------------------\n");
			printf("pass the goal: %d cost: %d times\n", current_goal, count);
			printf("-------------------------------------\n");
			current_goal++;
			pid_ctrl.init_errorsum();
		}

		//break when the pioneer arrives at the last goal
		if (current_goal == path.size()){
			printf("-------------------------------------------\n");
			printf("Arrive at the final point!! %d times costed\n", count);
			printf("-------------------------------------------\n");
			return 0;
		}

		float ctrl = pid_ctrl.get_control(car_pose, path[current_goal]);
		//drive_msg_stamped.drive.speed = pow((2 - fabs(ctrl)),2) + 0.7;
		drive_msg_stamped.drive.speed = (M_PI - fabs(ctrl))/2 + 1.0; 
		drive_msg_stamped.drive.steering_angle = ctrl;
		car_ctrl_pub.publish(drive_msg_stamped);
		
        ros::spinOnce();
        control_rate.sleep(); 
		printf("ctrl(steering) : %.2f distance: %.2f\n", ctrl, distance);
...
void setpath(){
    ...
    point point0;    point0.x = -4.0;    point0.y = -4.0;    point0.th = 0.5 * M_PI;
